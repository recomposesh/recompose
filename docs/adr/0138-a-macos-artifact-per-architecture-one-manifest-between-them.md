...
architecture. The automatic-updates brainstorm on 2026-08-17 revisited the choice. `Squirrel.Mac`
carries no differential path, so macOS re-downloads the whole application on every update, and a
universal bundle carries both architectures in every one of those downloads.
...
a selection it has carried since 4.3.7, well below the pinned 6.x.
...
**Good**: every macOS download halves, including the full re-download each update costs. The Intel
gap closes. One manifest leaves the release run, so nothing merges and nothing overwrites.
