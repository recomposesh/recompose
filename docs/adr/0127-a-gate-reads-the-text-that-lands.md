...
The ordering mechanism turned out not to be the one the file advertised. Every job carried a `priority` key, and the `priority` documentation for lefthook 2.1.10 describes a sort where lower numbers run first and `0` runs last. That option belongs to the legacy `commands:` and `scripts:` maps, which hold no order of their own and so need a sort key. It doesn't reach the `jobs:` array. The `Job` struct in `internal/config/job.go` at tag v2.1.10 carries no `Priority` field at all, while `Command` in `command.go` carries both the field and the comparator that honors it.
