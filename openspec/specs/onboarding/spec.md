...
The behavioral contract of a first session: the wizard that meets a new profile, and what it
leaves behind. A profile that has never finished it and never dismissed it opens under a surface
holding the whole window, and every other launch falls straight into the canvas. The wizard asks
three questions and builds the graph. It hands the window back with one virtual model standing
behind one round-robin router inside one running gateway. It reads what the machine already holds
rather than asking a person to type it, and every step it takes is one they could take by hand.

Nothing inside the surface dismisses it. Leaving is a control somebody took on purpose, and it
records a standing that never returns on its own. The View menu is the way back. The last step
waits on a request the gateway serves rather than on anything the app can arrange. The note that
meets that request stands on the canvas rather than over it.
