...
- [ ] Task 1: the contract vocabulary. Owns `packages/contracts/` alone and hands off the
      widened enums, `view:command`, `system:surface-toggles`, `usageSearchRangeSchema`,
      `fileBrowserSchema`, and `revealLabelFor`.
- [ ] Task 2: the lifecycle extraction and the Dock. Depends on task 1, owns
...
- [ ] Task 4: the renderer answers and the bridge. Depends on task 1, owns
...
- [ ] Task 5: the end-to-end layer. Depends on tasks 3 and 4, owns `apps/desktop/e2e/`, and
      hands off the two harness readers and the graduated scenarios.
- [ ] Task 6: the three decision records. Depends on task 3, owns `docs/adr/`, and hands off
      the landed records.
