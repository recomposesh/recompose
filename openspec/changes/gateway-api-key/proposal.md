...
6. **The comparison is constant time over digests.** Both sides hash to SHA-256 before
   `timingSafeEqual`, so the compare leaks neither the length of the key nor the length of its
   matching prefix.
