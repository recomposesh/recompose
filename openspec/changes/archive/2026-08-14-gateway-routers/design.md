...
**Amended 2026-08-15.** The paragraph above named the wrong file and the wrong condition, and the implementation follows the proposal, which outranks the design. `gateway-request-crossing.ts` decides nothing about a chained turn. It refuses only for a binding no walk could cure, ahead of the first child. The walk in `packages/engine/src/routing/attempt-walk.ts` refuses at the router about to rotate, at whatever depth it reached that router. `packages/engine/src/gateway-walk-answer.ts` then names that router rather than the one the table opens with. Routers chain, so the router that would spread the turn sits wherever the walk reaches it, not at the table's entry. A condition written against the entry lets a nested round-robin rotate a chained turn without refusing it. Everything else the paragraph says still stands: the status, the remedy sentence, failover serving a chain in declared order, and the deferral to issue #45.
