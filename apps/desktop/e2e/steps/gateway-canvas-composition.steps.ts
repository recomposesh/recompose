...
Then(
  ...
  async ({ page }, name: string) => {
    ...
    await expect(canvasNode(page, targetNodeId(name))).toBeVisible();
  },
);
...
Then(
  ...
  async ({ page }, wiredToTheGateway: string, cabledToTheAccount: string) => {
    ...
    await expect(
      cableBetween(page, modelNodeId(cabledToTheAccount), targetNodeId(cabledToTheAccount)),
    ).toHaveCount(1);
  },
);
...
Then('no node stands for that account', async ({ page }) => {
  ...
  expect(standing.filter((id) => id.startsWith('target:'))).toEqual([]);
  expect(standing.filter((id) => id.startsWith('ghost:'))).toEqual([]);
});
