...
/**
 * The three desks that fold what the child says on its own into what the windows are told.
 *
 * @summary They stand together because every one of them answers the same four moments in a
 * gateway's life, and a caller that had to remember all three by name would eventually tell two of
 * them a gateway stopped and forget the third.
 */
...
export function aDeskHeardIt(desks: EngineDesks, message: unknown): boolean {
  return desks.traffic.hears(message) || desks.pins.hears(message) || desks.logs.hears(message);
}
 ...
 * are simply gone: the branch a conversation earned lived in the engine child's memory and stopped
 * with it, so a count kept here would say conversations are held where nothing is.
 */
