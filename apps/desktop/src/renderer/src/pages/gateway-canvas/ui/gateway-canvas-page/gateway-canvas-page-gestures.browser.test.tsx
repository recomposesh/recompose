...
test('a cable from the gateway let go on open canvas births a draft where it landed', async () => {
  ...
  await expect.poll(() => cardCornerOn(screen.container, 'draft')).toEqual(letGo);
});
...
test('a press that lands on a card frame rather than the card leaves the selection alone', async () => {
  ...
  await clickedNodeFrame(screen.container, 'target:k1');

  await expect.poll(() => pressedStateOf(screen.container, 'target:k1')).toBe('false');
...
test('a pane press while the picker stands leaves the pick standing', async () => {
  const screen = await canvasPageOn();
...
test('a dragged draft card keeps the seat the drag left it at', async () => {
  ...
  await expect.poll(() => seatOf(screen.container, 'draft')).toBe('translate(320px, 280px)');

  draggedCard(cardWrapper(screen.container, 'draft'), { x: 40, y: 30 });

  await expect.poll(() => seatOf(screen.container, 'draft')).toBe('translate(360px, 310px)');
});
...
test('tidying seats a pending card in the target column with the cards it waits beside', async () => {
  ...
  await pulledCable(await sourcePortOf(screen.container, 'model:creative'), spot);
  releasedAt(spot);
  await expect.poll(() => cardCornerOn(screen.container, 'pending')).toEqual({ x: 520, y: 420 });
